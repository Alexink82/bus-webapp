...
- **Осталось:** инфраструктура (ротация логов, опционально CORS); P0 (audit log ролей); P1 (role-shell, единый feedback); P2 и прочее (память маршрута, FAQ BE, UI_IMPROVEMENTS, идемпотентность, единый стиль админки, WebPay, кэш маршрутов). Полный список — в разделе **«Осталось сделать»** выше.
