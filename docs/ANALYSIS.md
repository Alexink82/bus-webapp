...
## Правки по согласованному анализу (март 2026)

- **Главная (index):** Под подсказкой к форме добавлен текст «Время отправления и цена появятся после нажатия «Найти рейсы»» (ключ i18n `findTripsHint`). Сообщения «рейсов нет» и «маршрут не найден» заменены на развёрнутые формулировки (`noTripsForDate`, `routeNotFound`) с подсказкой выбрать другую дату/направление.
- **Тесты:** Добавлен `test_create_booking_validation_invalid_phone` — проверка отклонения некорректного телефона при создании брони (локальный маршрут).
