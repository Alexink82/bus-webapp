(function() {
  ...
  var ROUTES_FETCH_TIMEOUT_MS = 22000;
  ...
  function showRoutesError(msg) {
    ...
    var text = msg || 'Не удалось загрузить маршруты. Проверьте интернет или откройте позже.';
    var html = '<div class="booking-error-block"><p class="field-error">' + text + '</p><p><button type="button" class="btn btn-primary" id="retryRoutesBtn">Повторить</button> <a href="index.html" class="btn btn-outline">Выбрать маршрут</a></p></div>';
    if (list) list.innerHTML = html;
    var retryBtn = document.getElementById('retryRoutesBtn');
    if (retryBtn) retryBtn.addEventListener('click', function() { window.location.reload(); });
    if (typeof showAppAlert === 'function') showAppAlert(text, 'Ошибка');
    if (typeof updateBookingUIForStep === 'function') updateBookingUIForStep(1);
  }
  var safetyTimeoutId = setTimeout(function() {
    var loadingEl = document.getElementById('passengersListLoading');
    if (loadingEl && loadingEl.parentNode) showRoutesError('Загрузка занимает слишком долго. Проверьте интернет и нажмите «Повторить» или выберите маршрут заново.');
  }, 25000);
  function clearSafetyTimeout() { if (safetyTimeoutId) { clearTimeout(safetyTimeoutId); safetyTimeoutId = null; } }
  fetch(base, { signal: controller.signal }).then(function(r) {
    ...
    clearSafetyTimeout();
    showRoutesError(err && err.name === 'AbortError' ? 'Сервер не ответил вовремя. Нажмите «Повторить» или выберите маршрут заново.' : null);
  });
  ...
  document.getElementById('fillFromProfile').addEventListener('click', function() {
    fillFromSavedPassengers();
  });
  ...
  getEl('phone').addEventListener('input', function() {
    clearStep2Errors();
    var countryEl = getEl('phoneCountry');
    var countryCode = countryEl ? countryEl.value : 'BY';
    if (typeof formatPhoneInputLocal === 'function') {
      var v = formatPhoneInputLocal(this.value, countryCode);
      if (v !== this.value) this.value = v;
    }
