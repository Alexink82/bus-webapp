...
Дополнительно для режима обслуживания: **MAINTENANCE_UNTIL** — дата/время в ISO (UTC), до которого `/api/health` возвращает 503 (см. раздел «CORS и переменные для продакшена»).
