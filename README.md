...
- **Rate limit:** ограничение запросов по IP хранится в памяти процесса. При нескольких воркерах Render каждый воркер ведёт свой счётчик — эффективный лимит умножается на число воркеров. При перезапуске счётчики обнуляются. Для строгого лимита в распределённой среде нужен Redis или аналог.
